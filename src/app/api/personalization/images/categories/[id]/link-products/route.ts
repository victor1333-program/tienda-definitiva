...
export async function GET(
  ...
  try {
    ...
    const resolvedParams = await params;// Obtener todas las imágenes de esta categoría y sus productos vinculados
...
export async function POST(
  ...
  try {
    ...
    // Obtener todas las imágenes de esta categoría
    const images = await prisma.personalizationImage.findMany({
      where: { categoryId: resolvedParams.id },
      select: { id: true }
    })
    ...
    if (images.length === 0) {
      return NextResponse.json(
        { error: 'No hay imágenes en esta categoría para vincular' },
        { status: 400 }
      )
    }
    ...
    if (linkToAll) {
      ...
      // Eliminar vínculos existentes de todas las imágenes de esta categoría
      ...
      return NextResponse.json({
        message: `Categoría vinculada a ${personalizableProducts.length} productos personalizables`,
      ...
      if (productIds.length === 0) {
        ...
        return NextResponse.json({
          message: 'Todos los vínculos de la categoría han sido eliminados',
          linkedCount: 0
        })
      }
      ...
      return NextResponse.json({
        message: `Categoría vinculada a ${products.length} producto(s)`,
    ...
    console.error('Error linking products to category:', error)
    return NextResponse.json(
